...
| 項目 | 必須 | 形式 | 使い道 |
| --- | --- | --- | --- |
| プロジェクト概要 | はい | 複数行テキスト | 目的セクションのたたき台 |
| 技術スタック | いいえ | 1行1項目 | 技術スタックと技術固有ルールの候補 |
| 追加で守ってほしいこと | いいえ | 1行1項目 | 制約の候補 |

初回画面に、実行コマンド、プロジェクト名、Git方針などの詳細入力は置かない。これらは次画面で、たたき台を見ながら任意で編集する。

初期リリースでは文字数・件数の固定上限を設けない。ブラウザ内で保存・外部送信をしないため、正規化では有効な入力を切り詰めず、型不正、空値、項目ごとの改行制約だけを扱う。保存や外部APIを追加する場合は、その制約に合わせて上限を設計する。
...
固定ルールは以下のカテゴリと順序でたたき台へ入れ、最終Markdownでも対応するセクションへ同じ順序で出力する。利用者は各カテゴリで最後の1件を残したまま編集・追加できる。
...
- 箇条書きは `- ` を使う。
- 利用者が編集した通常テキスト（タイトル、目的、技術スタック、追加制約、コマンド用途ラベル、共通ルール、技術固有ルール）は、Markdownとして解釈しない。正規化後、バックスラッシュを先に `\\` へエスケープし、その後 `` ` ~ = * _ { } [ ] ( ) < > # + - . ! | & `` の各文字の直前へ `\` を付ける。複数行の目的は各行の先頭・末尾の半角空白とタブを除去してからこの処理を行い、改行を維持する。アプリの固定文言とコマンド本文はこの通常テキストのエスケープ対象外とする。
- 文書タイトルは、前後の空白を除去した単一行として扱う。空の場合は入力エラーにせず `AGENTS.md` へ正規化し、CR・LFを含む値は保存せず該当入力の近くにエラーを表示する。目的とタイトル以外の通常テキストも、前後の空白を除去した単一行だけを受け付ける。空文字列またはCR・LFを含む値は追加・保存せず、該当入力の近くにエラーを表示する。
- コマンドは、前後の空白を除去した単一行文字列だけを受け付ける。空文字列またはCR・LFを含む値は追加・保存せず、該当入力の近くにエラーを表示する。出力時はインラインコードとして扱い、必要に応じて編集済みの用途ラベルを先頭へ付ける。区切りには、コマンド値内の最長の連続バッククォートより1個長いバッククォート列を使い、区切りの内側へ半角スペースを1つ置く。
