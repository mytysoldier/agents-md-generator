  ...
  return (
    <section className="editor-section" aria-labelledby="document-title-heading">
      ...
      <Field id="edited-project-summary" label="プロジェクトの目的" required>
        <textarea id="edited-project-summary" required value={draft.projectSummary} onChange={(event) => updateProjectSummary(event.target.value)} rows={5} className="field" aria-invalid={Boolean(projectSummaryError)} aria-describedby={projectSummaryError ? 'edited-project-summary-error' : undefined} />
      </Field>
